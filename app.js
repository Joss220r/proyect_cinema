...
createApp({
  data(){
    return {
      ...
      cargando: false,
      error: '',
      filtros: { title:'', ubication:'', type:'', onlyActive:false, sort:'desc' },
      opciones: { ubications:[], types:[] },
      pagina: 1,
      porPagina: 12,
      seleccionado: null,
      admin: {
        create: { imdbID:'', Title:'', Year:'', Type:'', Poster:'', Estado:true, description:'', Ubication:'' },
        update: { imdbID:'', Title:'', Year:'', Type:'', Poster:'', Estado:true, description:'', Ubication:'' },
        toDelete: ''
      }
    };
  },
  computed:{
    procesadas(){
      ...
      if(this.filtros.type) arr = arr.filter(m => (m.Type||'') === this.filtros.type);
      if(this.filtros.onlyActive) arr = arr.filter(m => !!m.Estado);
      arr.sort((a,b)=>{
        const ay = parseInt((a.Year||'').toString().match(/\d{4}/)?.[0]||'0',10);
        const by = parseInt((b.Year||'').toString().match(/\d{4}/)?.[0]||'0',10);
        return this.filtros.sort==='asc' ? ay - by : by - ay;
      });
      return arr;
    },
    totalEncontradas(){ return this.peliculas.length; },
    totalPaginas(){ return Math.max(1, Math.ceil(this.procesadas.length / this.porPagina)); },
    paginadas(){
  ...
  methods:{
    ...
    async buscarServidor(){
      this.cargando = true; this.error=''; this.pagina=1;
      try{
